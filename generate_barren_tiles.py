...
for x in range(border_size, width - border_size):
    for y in range(border_size, height - border_size):
        if (x, y) not in visited and pixels[x, y] != base_color:
            ...
            # Only keep very small tufts for barren tiles
            if len(stamp_pixels) < 8:
                stamps.append((normalized_stamp, sw, sh))

# Generate 3 Barren variations
for var_idx in range(1, 4):
    new_img = Image.new('RGBA', (width, height))
    new_pixels = new_img.load()
    
    # Fill with base color (Flat)
    for x in range(width):
        for y in range(height):
    ...
    # For barren tiles, we place VERY few tufts (0 to 2) and only use small stamps
    num_stamps_to_place = random.randint(0, 2)
    placed_stamps = []
    ...
    out_path = f'Assets/05_Tiles/Grass/Summer/Summer_v1_f1_barren{var_idx}.png'
    new_img.save(out_path)
    print(f"Saved barren variation to {out_path}")
