...
# Extract stamps (connected components of non-base colors)
# We only extract stamps from the interior to avoid cutting off border pixels
border_size = 3
visited = set()
stamps = []
...
for x in range(border_size, width - border_size):
    for y in range(border_size, height - border_size):
        if (x, y) not in visited and pixels[x, y] != base_color:
            ...
            # Normalize stamp coordinates so the top-left is (0,0)
...
for var_idx in range(1, 4):
    ...
    num_stamps_to_place = random.randint(4, 7)
    placed_stamps = []
    
    # Try placing stamps randomly without overlap
    for _ in range(num_stamps_to_place * 3): # attempts
        if not stamps: break
    ...
    print(f"Saved softer variation to {out_path}")
