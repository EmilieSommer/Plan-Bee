...
public static class RoomTileSetup
{
    ...
    static void SetupBorder(string folder, string output)
    {
        ...
        // All-neighbor tile = floor → set as default, skip rule
        ...
        Save(tile, output);
        Debug.Log($"[Plan Bee] Border {Path.GetFileName(folder)}: {tile.m_TilingRules.Count} rules → {output}");
    }
    ...
    static void SetupOverlay(string folder, string output)
    {
        ...
        // inside.png (no letters, key all-false) = fully surrounded = transparent default
        ...
        {
            var (t, b, l, r) = kv.Key;
            // Letter present = that side has a different neighbor = NotThis
            tile.m_TilingRules.Add(MakeRule(kv.Value,
                t ? NOT : THIS,
                b ? NOT : THIS,
                l ? NOT : THIS,
        ...
        Save(tile, output);
        Debug.Log($"[Plan Bee] Overlay {Path.GetFileName(folder)}: {tile.m_TilingRules.Count} rules → {output}");
    }
        ...
        string folder, bool isOverlay)
    {
        ...
        foreach (var fullPath in Directory.GetFiles(folder, "*.png"))
        {
            ...
            // Overlay: strip "inside_" prefix; bare "inside" → empty string (transparent default)
            if (isOverlay)
            {
                if (name == "inside")          name = "";
