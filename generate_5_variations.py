...
# Extract soft tufts (stamps) from the original interior
...
# Generate 5 variations ranging from very barren to very dense
stamp_counts = [1, 3, 6, 9, 12]
...
for i, num_stamps_to_place in enumerate(stamp_counts):
    ...
    # Copy exactly 1-pixel borders perfectly to ensure seamless tiling
    for x in range(width):
        for y in range(height):
    ...
    print(f"Saved soft variation {var_idx} (density: {num_stamps_to_place} tufts) to {out_path}")
